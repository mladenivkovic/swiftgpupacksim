...
layouts, making a single output image per experiment and per thread and
by whether flush or noflush options were used. Which experiments to
    ...
    for flush_suffix in dir_suffixes_flush:
        ...
        for nthreads in NRTHREADS:
            for name in EXPERIMENT_NAMES:
                ...
                ax5.set_title("gradient/unpacking")
                ax6 = fig.add_subplot(2, 3, 6)
                ax6.set_title("force/unpacking")
                ...
                outfname = f"struct_alignment_flushexp_{name}_{srcdir}_{nthreads}threads{flush_suffix}"
                if args.png:
                    outfname += ".png"
                else:
                    outfname += ".pdf"
