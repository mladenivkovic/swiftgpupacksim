...
parser = argparse.ArgumentParser(
    ...
    description="""
Compare results of manually vectorized vs not for different loop splitting
variants and different particle memory layouts for a given experiment and a
given particle accessor variant.
...
variants = [variant_dir_suffix, variant_dir_suffix + "_vector"]
variants_labels = [variant_label_suffix, variant_label_suffix + " with simd pragma"]
...
if __name__ == "__main__":
    ...
    for experiment in EXPERIMENTS:
        for a, access in enumerate(PART_ACCESS):
            ...
            for v, variant in enumerate(variants):
                ...
                for s, split in enumerate(LOOP_SPLITS):
                    ...
                    for l, layout in enumerate(layouts):
                        fname = get_result_fname(srcdir, experiment, nthreads, access, split, variant, layout)
                    ...
                    label = PART_ACCESS_LABELS[a] + " " + LOOP_SPLIT_LABELS[s] + variants_labels[v]
            ...
            outfname = f"loop_splitting_compare_vector_{srcdir}_{experiment}_{access}"
