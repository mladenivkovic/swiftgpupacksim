...
parser = argparse.ArgumentParser(
    ...
    description="""
Compare results of using different kinds of flushes between copy operations for
a given experiment, given loop splitting variant, and a given particle accessor variant.
(Basically: Plot results of job_flush_[NODE].slm)

By default, this plots the outputs using:
- not manually encouraged vectorisation
- 72 threads
- not packed structs
- explicit-var particle access
- no loop splitting
...
<EXPERIMENTNAME>_<NRTHREADS>threads_<PART_ACCESS>_<LOOP_SPLIT>[_<FLUSH_VARIANT>][_vector][_packed]
...
FLUSH_VARIANTS = ["noflush", "flush", "bigflush", "x86flush"]
FLUSH_VARIANT_LABELS = ["no flush", "2MB array", "10MB array", "x86 intrinsics"]

variant_dir_suffix, variant_label_suffix = get_variant_labels(
    False, args.use_vector, args.use_packed
)
VARIANTS = [variant_dir_suffix + "_" + f for f in FLUSH_VARIANTS]
VARIANT_LABELS = [variant_label_suffix + " " + f for f in FLUSH_VARIANT_LABELS]
...
if __name__ == "__main__":
    ...
    firstdir = get_result_dir(
        srcdir, EXPERIMENTS[0], nthreads, access_variant, loop_split, "noflush"
    )
    ...
    # get index of aos data for normalisation
    aos_ind = -1
    ...
    for e, experiment in enumerate(EXPERIMENTS):
        ...
        for v, variant in enumerate(VARIANTS):
            ...
            if os.path.exists(fdir) and os.path.isdir(fdir):
                print(experiment, ": found flush variant", variant)
            else:
                print(experiment, ": DO NOT HAVE flush variant", variant)
                continue
