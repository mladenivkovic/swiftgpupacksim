...
void gpu_data_init_pack_arrays(struct pack_vars_self **pack_vars_self_dens,
  ...
  if (verbose)
    message(
        "cudaMallocHost pack_vars bundle_first size=%lu total=%lu nBundles=%d",
        nBundles * sizeof(int), 27 * nBundles * sizeof(int), nBundles);
  ...
  if (verbose)
    message("Calloc dens self task_list size1=%lu size2=%lu target_n_tasks %d",
            ...
            target_n_tasks * sizeof(struct cell *), target_n_tasks);
  total += target_n_tasks * sizeof(struct task *) +
  ...
  /*Allocate target_n_tasks for top level tasks. This is a 2D array with length
   * target_n_tasks and width n_leaves_max*/
  (*pack_vars_pair_dens)->leaf_list = (struct leaf_cell_list *)calloc(
      target_n_tasks, sizeof(struct leaf_cell_list));

  if (verbose)
    message(
        "Malloc dens pair leaf_list size1=%lu size2=%lu total=%lu "
        "target_n_tasks %d n_leaves_max %d",
        n_leaves_max * sizeof(struct cell *), n_leaves_max * sizeof(double),
        target_n_tasks * (2 * n_leaves_max * sizeof(struct cell *) +
                          3 * n_leaves_max * sizeof(double)),
        target_n_tasks, n_leaves_max);
  total += target_n_tasks * (2 * n_leaves_max * sizeof(struct cell *) +
                             3 * n_leaves_max * sizeof(double));

  for (int i = 0; i < target_n_tasks; i++) {
    (*pack_vars_pair_dens)->leaf_list[i].ci =
        (struct cell **)malloc(n_leaves_max * sizeof(struct cell *));
    (*pack_vars_pair_dens)->leaf_list[i].cj =
        (struct cell **)malloc(n_leaves_max * sizeof(struct cell *));
    (*pack_vars_pair_dens)->leaf_list[i].shiftx =
        (double *)malloc(n_leaves_max * sizeof(double));
    (*pack_vars_pair_dens)->leaf_list[i].shifty =
        (double *)malloc(n_leaves_max * sizeof(double));
    (*pack_vars_pair_dens)->leaf_list[i].shiftz =
        (double *)malloc(n_leaves_max * sizeof(double));
  ...
  if (verbose)
    message("Calloc dens pair cell lists size=%lu total=%lu target_n_tasks=%d",
            target_n_tasks * sizeof(struct cell *),
            2 * target_n_tasks * sizeof(struct cell *), target_n_tasks);
  ...
  if (verbose)
    message(
        "Calloc force self cell lists size1=%lu size2=%lu total=%lu "
        "target_n_tasks=%d",
        target_n_tasks * sizeof(struct task *),
        target_n_tasks * sizeof(struct cell *),
        target_n_tasks * sizeof(struct task *) +
  ...
  if (verbose)
    message(
        "Calloc force pair cell lists size1=%lu size2=%lu total=%lu "
        "target_n_tasks=%d",
        target_n_tasks * sizeof(struct task *),
        target_n_tasks * sizeof(struct cell *),
        target_n_tasks * sizeof(struct task *) +
  ...
  if (verbose)
    message(
        "Calloc grad self cell lists size1=%lu size2=%lu total=%lu "
        "target_n_tasks=%d",
        target_n_tasks * sizeof(struct task *),
        target_n_tasks * sizeof(struct cell *),
        target_n_tasks * sizeof(struct task *) +
  ...
  if (verbose)
    message(
        "Calloc grad pair cell lists size1=%lu size2=%lu total=%lu "
        "target_n_tasks=%d",
        target_n_tasks * sizeof(struct task *),
        target_n_tasks * sizeof(struct cell *),
        target_n_tasks * sizeof(struct task *) +
  ...
  if (verbose) message("Total alloc'd memory: %lu", total);
}


void gpu_data_init_first_part_host_arrays(int2 **task_first_part_f4,
  ...
  if (verbose)
    message(
        "cudaMallocHost first_part arrays size=%lu total = %lu target_n_tasks "
        "= %d",
        target_n_tasks * sizeof(int2), 3 * target_n_tasks * sizeof(int2),
        target_n_tasks);

  err = cudaMallocHost((void **)task_first_part_f4,
                       target_n_tasks * sizeof(int2));
  swift_assert(err == cudaSuccess);

  /* err = cudaMalloc((void **)d_task_first_part_f4, target_n_tasks *
   * sizeof(int2)); */
  /* swift_assert(err == cudaSuccess); */

  err = cudaMallocHost((void **)task_first_part_f4_f,
                       target_n_tasks * sizeof(int2));
  swift_assert(err == cudaSuccess);

  /* err = cudaMalloc((void **)d_task_first_part_f4_f, target_n_tasks *
   * sizeof(int2)); */
  /* swift_assert(err == cudaSuccess); */

  err = cudaMallocHost((void **)task_first_part_f4_g,
                       target_n_tasks * sizeof(int2));
  swift_assert(err == cudaSuccess);

  /* err = cudaMalloc((void **)d_task_first_part_f4_g, target_n_tasks *
   * sizeof(int2)); */
  /* swift_assert(err == cudaSuccess); */
}
...
void gpu_data_init_send_recv_host_arrays(
    ...
    const int count_max_parts_tmp, const int verbose) {

  cudaError_t err = cudaSuccess;

  if (verbose)
    message(
        "cudaMallocHost parts_aos_f4 size=%lu total=%lu count_max_parts_tmp=%d",
...
void test_alloc_and_free(int **test2) {
  ...
  /* cudaError_t err = cudaSuccess; */
  err = cudaMallocHost((void **)&test_p, sizeof(struct pack_vars_self));
  printf("Check1\n");
  fflush(stdout);

  err = cudaMallocHost((void **)test2, 10 * sizeof(int));
  printf("Check2\n");
  fflush(stdout);
...
void gpu_data_clear_first_part_host_arrays(int2 **task_first_part_f4,
                                           int2 **task_first_part_f4_g,
                                           int2 **task_first_part_f4_f) {
  /* int2 **d_task_first_part_f4, */
  /* int2 **d_task_first_part_f4_g, */
