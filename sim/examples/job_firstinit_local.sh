...
for part_access in "explicit-var"; do
  for loop in "none"; do
    ...
    for dir in IntelCoffeeLake_Gresho128_6threads; do
      ...
      mkdir -p $NODE/"$dir"_"$part_access"_"$loop"_firstinit
      ...
      for layout in aos soa upstream pack-gradient pack-force pack-shared; do
        ...
        ex="likwid-pin --quiet -c N:0-""$((NTHREADS - 1))"" ../swiftgpupack_PARALLELINIT_""$NODE"_"$layout"_"$part_access"_"$loop"" -s 2"
        outfile=$NODE/"$dir"_"$part_access"_"$loop"_firstinit/results_"$layout".csv
        if [[ ! -f "$outfile" ]]; then
          echo running "$ex" ../../data/$dir
          $ex ../../data/$dir
          mv results_"$layout".csv "$outfile"
