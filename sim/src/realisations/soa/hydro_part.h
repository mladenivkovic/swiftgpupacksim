/**
  ...
  * File template is /cosma/home/do009/dc-ivko1/swiftgpupacksim-gn002/py/templates/hydro_part.h.jinja.template
  */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_density1* part_density1_s = p->_cell_part_arrays->_part_density1 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_density1* part_density1_s = p->_cell_part_arrays->_part_density1 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force1* part_force1_s = p->_cell_part_arrays->_part_force1 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force1* part_force1_s = p->_cell_part_arrays->_part_force1 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_density2* part_density2_s = p->_cell_part_arrays->_part_density2 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_density2* part_density2_s = p->_cell_part_arrays->_part_density2 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force2* part_force2_s = p->_cell_part_arrays->_part_force2 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force2* part_force2_s = p->_cell_part_arrays->_part_force2 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_density3* part_density3_s = p->_cell_part_arrays->_part_density3 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_density3* part_density3_s = p->_cell_part_arrays->_part_density3 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force3* part_force3_s = p->_cell_part_arrays->_part_force3 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force3* part_force3_s = p->_cell_part_arrays->_part_force3 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_density4* part_density4_s = p->_cell_part_arrays->_part_density4 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE const float*
  ...
  const struct part_density4* part_density4_s = p->_cell_part_arrays->_part_density4 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force4* part_force4_s = p->_cell_part_arrays->_part_force4 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force4* part_force4_s = p->_cell_part_arrays->_part_force4 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force5* part_force5_s = p->_cell_part_arrays->_part_force5 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force5* part_force5_s = p->_cell_part_arrays->_part_force5 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct part_force6* part_force6_s = p->_cell_part_arrays->_part_force6 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct part_force6* part_force6_s = p->_cell_part_arrays->_part_force6 + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
#endif /* SWIFT_HYDRO_PART_fdb020f32f5276b4daa9db133b562dec984feedd_H */
