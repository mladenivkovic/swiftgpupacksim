/**
  ...
  * File template is /cosma/home/do009/dc-ivko1/swiftgpupacksim-gn002/py/templates/parts.h.jinja.template
  */
#pragma once
...
static __attribute__((always_inline)) INLINE void init_part_arrays(struct hydro_part_arrays* parr, int nr_parts) {
  ...
  if (swift_memalign("parts_part_density1", (void**)&parr->_part_density1, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_density1)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_density1, 0, nr_parts * sizeof(struct part_density1));

  if (swift_memalign("parts_part_force1", (void**)&parr->_part_force1, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force1)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force1, 0, nr_parts * sizeof(struct part_force1));

  if (swift_memalign("parts_part_density2", (void**)&parr->_part_density2, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_density2)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_density2, 0, nr_parts * sizeof(struct part_density2));

  if (swift_memalign("parts_part_force2", (void**)&parr->_part_force2, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force2)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force2, 0, nr_parts * sizeof(struct part_force2));

  if (swift_memalign("parts_part_density3", (void**)&parr->_part_density3, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_density3)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_density3, 0, nr_parts * sizeof(struct part_density3));

  if (swift_memalign("parts_part_force3", (void**)&parr->_part_force3, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force3)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force3, 0, nr_parts * sizeof(struct part_force3));

  if (swift_memalign("parts_part_density4", (void**)&parr->_part_density4, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_density4)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_density4, 0, nr_parts * sizeof(struct part_density4));

  if (swift_memalign("parts_part_force4", (void**)&parr->_part_force4, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force4)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force4, 0, nr_parts * sizeof(struct part_force4));

  if (swift_memalign("parts_part_force5", (void**)&parr->_part_force5, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force5)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force5, 0, nr_parts * sizeof(struct part_force5));

  if (swift_memalign("parts_part_force6", (void**)&parr->_part_force6, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct part_force6)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_part_force6, 0, nr_parts * sizeof(struct part_force6));
  ...
  for (long long i = 1; i <= nr_parts; i++) {
    ...
    parr->_part_density1[i-1]._accessor_id = i;
    parr->_part_force1[i-1]._accessor_id = i;
    parr->_part_density2[i-1]._accessor_id = i;
    parr->_part_force2[i-1]._accessor_id = i;
    parr->_part_density3[i-1]._accessor_id = i;
    parr->_part_force3[i-1]._accessor_id = i;
    parr->_part_density4[i-1]._accessor_id = i;
    parr->_part_force4[i-1]._accessor_id = i;
    parr->_part_force5[i-1]._accessor_id = i;
    parr->_part_force6[i-1]._accessor_id = i;
  ...
  total+=sizeof(struct part_density1);
  total+=sizeof(struct part_force1);
  total+=sizeof(struct part_density2);
  total+=sizeof(struct part_force2);
  total+=sizeof(struct part_density3);
  total+=sizeof(struct part_force3);
  total+=sizeof(struct part_density4);
  total+=sizeof(struct part_force4);
  total+=sizeof(struct part_force5);
  total+=sizeof(struct part_force6);
...
static __attribute__((always_inline)) INLINE void clear_parts(struct hydro_part_arrays* parr) {
  ...
  free(parr->_part_density1);
  free(parr->_part_force1);
  free(parr->_part_density2);
  free(parr->_part_force2);
  free(parr->_part_density3);
  free(parr->_part_force3);
  free(parr->_part_density4);
  free(parr->_part_force4);
  free(parr->_part_force5);
  free(parr->_part_force6);
...
__attribute__((always_inline)) INLINE static void part_arrays_set_pointer_offset(
  ...
  dest->_part_density1 = &src->_part_density1[offset];
  dest->_part_force1 = &src->_part_force1[offset];
  dest->_part_density2 = &src->_part_density2[offset];
  dest->_part_force2 = &src->_part_force2[offset];
  dest->_part_density3 = &src->_part_density3[offset];
  dest->_part_force3 = &src->_part_force3[offset];
  dest->_part_density4 = &src->_part_density4[offset];
  dest->_part_force4 = &src->_part_force4[offset];
  dest->_part_force5 = &src->_part_force5[offset];
  dest->_part_force6 = &src->_part_force6[offset];
