...
static __attribute__((always_inline)) INLINE void init_part_arrays(struct hydro_part_arrays* parr, int nr_parts) {
  ...
  if (swift_memalign("parts_x_h_v_m_double", (void**)&parr->_x_h_v_m_double, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct x_h_v_m_double)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_x_h_v_m_double, 0, nr_parts * sizeof(struct x_h_v_m_double));
  ...
  if (swift_memalign("parts_force_pack_timebin", (void**)&parr->_force_pack_timebin, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct force_pack_timebin)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_force_pack_timebin, 0, nr_parts * sizeof(struct force_pack_timebin));

  if (swift_memalign("parts_force_pack_limiter", (void**)&parr->_force_pack_limiter, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct force_pack_limiter)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_force_pack_limiter, 0, nr_parts * sizeof(struct force_pack_limiter));

  if (swift_memalign("parts_force_gradient_pack_shared", (void**)&parr->_force_gradient_pack_shared, SWIFT_PART_ALIGNMENT, nr_parts * sizeof(struct force_gradient_pack_shared)) != 0)
    error("Error while allocating memory for SPH particles");

  memset(parr->_force_gradient_pack_shared, 0, nr_parts * sizeof(struct force_gradient_pack_shared));
  ...
  for (long long i = 1; i <= nr_parts; i++) {
    ...
    parr->_x_h_v_m_double[i-1]._accessor_id = i;
    parr->_x_h_v_m[i-1]._accessor_id = i;
    parr->_gradient_pack[i-1]._accessor_id = i;
    parr->_force_pack[i-1]._accessor_id = i;
    parr->_force_pack_timebin[i-1]._accessor_id = i;
    parr->_force_pack_limiter[i-1]._accessor_id = i;
    parr->_force_gradient_pack_shared[i-1]._accessor_id = i;
  ...
  total+=sizeof(struct x_h_v_m_double);
  total+=sizeof(struct x_h_v_m);
  total+=sizeof(struct gradient_pack);
  total+=sizeof(struct force_pack);
  total+=sizeof(struct force_pack_timebin);
  total+=sizeof(struct force_pack_limiter);
  total+=sizeof(struct force_gradient_pack_shared);
...
static __attribute__((always_inline)) INLINE void clear_parts(struct hydro_part_arrays* parr) {
  ...
  free(parr->_part);
  free(parr->_x_h_v_m_double);
  free(parr->_x_h_v_m);
  free(parr->_gradient_pack);
  free(parr->_force_pack);
  free(parr->_force_pack_timebin);
  free(parr->_force_pack_limiter);
  free(parr->_force_gradient_pack_shared);
...
__attribute__((always_inline)) INLINE static void part_arrays_set_pointer_offset(
  ...
  dest->_x_h_v_m_double = &src->_x_h_v_m_double[offset];
  dest->_x_h_v_m = &src->_x_h_v_m[offset];
  dest->_gradient_pack = &src->_gradient_pack[offset];
  dest->_force_pack = &src->_force_pack[offset];
  dest->_force_pack_timebin = &src->_force_pack_timebin[offset];
  dest->_force_pack_limiter = &src->_force_pack_limiter[offset];
  dest->_force_gradient_pack_shared = &src->_force_gradient_pack_shared[offset];
