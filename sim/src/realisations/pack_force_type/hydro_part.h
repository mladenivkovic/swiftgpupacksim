...
static __attribute__((always_inline)) INLINE float
  ...
  if(force_pack_s->_accessor_id != p->_accessor_id)
    error("Accessor IDs not equal: %lld %lld", force_pack_s->_accessor_id, p->_accessor_id);
#endif
  return force_pack_s->_soundspeed;
}
...
static __attribute__((always_inline)) INLINE void
  ...
  if(force_pack_s->_accessor_id != p->_accessor_id)
    error("Accessor IDs not equal: %lld %lld", force_pack_s->_accessor_id, p->_accessor_id);
#endif
  force_pack_s->_soundspeed = soundspeed;
}
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct density_unpack* restrict density_unpack_s = p->_cell_part_arrays->_density_unpack + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct density_unpack* restrict density_unpack_s = p->_cell_part_arrays->_density_unpack + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct density_unpack* restrict density_unpack_s = pd->_density_unpack + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + pind;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct density_unpack* restrict density_unpack_s = global_hydro_part_arrays._density_unpack + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = global_hydro_part_arrays._part + pind;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  if(density_unpack_s->_accessor_id != p->_accessor_id)
    error("Accessor IDs not equal: %lld %lld", density_unpack_s->_accessor_id, p->_accessor_id);
#endif
  return density_unpack_s->_rot_v[i];
}
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct force_unpack* restrict force_unpack_s = p->_cell_part_arrays->_force_unpack + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE const float*
  ...
  const struct force_unpack* restrict force_unpack_s = p->_cell_part_arrays->_force_unpack + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  if(force_unpack_s->_accessor_id != p->_accessor_id)
    error("Accessor IDs not equal: %lld %lld", force_unpack_s->_accessor_id, p->_accessor_id);
#endif
  return force_unpack_s->_a_hydro[i];
}
...
static __attribute__((always_inline)) INLINE const float*
  ...
  const struct force_unpack* restrict force_unpack_s = pd->_force_unpack + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + pind;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE const float*
  ...
  const struct force_unpack* restrict force_unpack_s = global_hydro_part_arrays._force_unpack + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = global_hydro_part_arrays._part + pind;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct others* restrict others_s = p->_cell_part_arrays->_others + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct others* restrict others_s = p->_cell_part_arrays->_others + p->_cell_offset;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  swift_assert(p->_accessor_id != 0);
  /* Make sure we're accessing the correct data */
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct others* restrict others_s = pd->_others + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + pind;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct others* restrict others_s = global_hydro_part_arrays._others + pind;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = global_hydro_part_arrays._part + pind;
  swift_assert(p->_accessor_id != 0);
