...
static __attribute__((always_inline)) INLINE double*
  ...
  struct x_h_v_m* restrict x_h_v_m_s = pd->_x_h_v_m + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE const double*
  ...
  const struct x_h_v_m* restrict x_h_v_m_s = pd->_x_h_v_m + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct gradient_pack* restrict gradient_pack_s = pd->_gradient_pack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct gradient_pack* restrict gradient_pack_s = pd->_gradient_pack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct force_pack* restrict force_pack_s = pd->_force_pack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct force_pack* restrict force_pack_s = pd->_force_pack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct force_gradient_pack_shared* restrict force_gradient_pack_shared_s = pd->_force_gradient_pack_shared + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct force_gradient_pack_shared* restrict force_gradient_pack_shared_s = pd->_force_gradient_pack_shared + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct density_unpack* restrict density_unpack_s = pd->_density_unpack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct density_unpack* restrict density_unpack_s = pd->_density_unpack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct force_unpack* restrict force_unpack_s = pd->_force_unpack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE const float*
  ...
  const struct force_unpack* restrict force_unpack_s = pd->_force_unpack + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float
  ...
  const struct others* restrict others_s = pd->_others + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
...
static __attribute__((always_inline)) INLINE float*
  ...
  struct others* restrict others_s = pd->_others + index;
#ifdef SWIFT_DEBUG_CHECKS
  /* Forbid ID = 0 to prevent false positives by forgotten initialisation */
  const struct part* restrict p = pd->_part + index;
  swift_assert(p->_accessor_id != 0);
